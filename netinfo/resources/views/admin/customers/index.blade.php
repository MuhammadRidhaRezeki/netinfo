...
@section('content')
    @php
        $custMeta = [
            ...
            'isolated' => ['label' => 'Isolated', 'pill' => 'bg-red-50 text-red-700 ring-red-600/20', 'dot' => 'bg-red-500'],
            'inactive' => ['label' => 'Inactive', 'pill' => 'bg-slate-100 text-slate-600 ring-slate-500/20', 'dot' => 'bg-slate-400'],
        ];
        $avatarColors = ['bg-indigo-100 text-indigo-700', 'bg-sky-100 text-sky-700', 'bg-violet-100 text-violet-700', 'bg-rose-100 text-rose-700', 'bg-amber-100 text-amber-700', 'bg-teal-100 text-teal-700'];
    @endphp
    ...
    <form method="GET" action="{{ route('admin.customers.index') }}" class="flex flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" onclick="openModal('modal-add-customer')" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-indigo-600/30 transition hover:bg-indigo-500">
            ...
            <table class="min-w-[900px] divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Kontak Login</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($customers as $i => $c)
                        <tr class="transition hover:bg-slate-50/70">
                            ...
                            <td class="whitespace-nowrap px-5 py-3.5">
                                <span class="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold ring-1 ring-inset {{ $custMeta[$c->status]['pill'] }}">
                            ...
                            <td class="whitespace-nowrap px-5 py-3.5 text-right">
                                <button type="button" title="Edit Data" data-edit-open data-url="{{ route('admin.customers.update', $c) }}"
                                    ...
                                    class="rounded-md p-1.5 text-slate-400 transition hover:bg-indigo-50 hover:text-indigo-600">
                                ...
                                @if ($c->status === 'active')
                                    <form method="POST" action="{{ route('admin.customers.status', $c) }}" class="inline" onsubmit="return confirm('Isolir pelanggan {{ $c->customer_code }}? Layanan akan ditangguhkan.')">
                                        ...
                                        <button type="submit" title="Isolir" class="rounded-md p-1.5 text-slate-400 transition hover:bg-red-50 hover:text-red-600">
                                ...
                                <form method="POST" action="{{ route('admin.customers.destroy', $c) }}" class="inline" onsubmit="return confirm('Hapus permanen pelanggan {{ $c->customer_code }} beserta akun loginnya?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" title="Hapus" class="rounded-md p-1.5 text-slate-300 transition hover:bg-red-50 hover:text-red-500">
                    ...
                    @empty
                        <tr><td colspan="7" class="px-5 py-10 text-center text-sm italic text-slate-400">Tidak ada pelanggan yang cocok dengan filter.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="border-t border-slate-200 bg-slate-50 px-5 py-3">
            <p class="text-xs text-slate-500">Menampilkan <span class="font-semibold">{{ $customers->count() }}</span> pelanggan @if($filters['q'] || $filters['status'])<span class="text-indigo-500">(terfilter)</span>@endif</p>
        </div>
    </div>
@endsection
...
@push('modals')
    <div id="modal-add-customer" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute left-1/2 top-1/2 max-h-[92vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between border-b border-slate-200 px-6 py-4">
                <div>
                    <h3 class="text-base font-bold text-slate-900">Tambah Pelanggan Baru</h3>
            ...
            <form method="POST" action="{{ route('admin.customers.store') }}" class="space-y-4 px-6 py-5">
                ...
                <div class="flex justify-end gap-2 border-t border-slate-200 pt-4">
                    <button type="button" onclick="closeModal('modal-add-customer')" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>
                    <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Simpan Pelanggan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-edit-customer" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute left-1/2 top-1/2 max-h-[92vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between border-b border-slate-200 px-6 py-4">
                <div>
                    <h3 class="text-base font-bold text-slate-900">Edit Data Pelanggan</h3>
            ...
            <form id="edit-customer-form" method="POST" action="#" class="space-y-4 px-6 py-5">
                ...
                <div class="flex justify-end gap-2 border-t border-slate-200 pt-4">
                    <button type="button" onclick="closeModal('modal-edit-customer')" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>
