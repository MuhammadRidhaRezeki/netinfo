...
@section('content')
    <div class="mb-3 flex items-start gap-2.5 rounded-xl border border-sky-200/90 bg-sky-50/40 px-4 py-3 text-sm text-sky-700">
    ...
    @php
        $custMeta = [
            ...
            'isolated' => ['label' => 'Isolated', 'pill' => 'bg-rose-50 text-rose-700 border-rose-200'],
            'inactive' => ['label' => 'Inactive', 'pill' => 'bg-slate-50 text-slate-600 border-slate-200'],
        ];
                    ...
                    @forelse ($customers as $i => $c)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="whitespace-nowrap px-4 py-3">
                                <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-medium {{ $custMeta[$c->status]['pill'] }}">{{ $custMeta[$c->status]['label'] }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="px-4 py-10 text-center text-sm italic text-slate-400">Tidak ada pelanggan sesuai filter.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
