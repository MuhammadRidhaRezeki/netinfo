...
@php
    ...
    $nodeMeta = [
        'active' => ['label' => 'Active', 'pill' => 'bg-emerald-50 text-emerald-700 ring-emerald-600/20', 'dot' => 'bg-emerald-500'],
        'maintenance' => ['label' => 'Maintenance', 'pill' => 'bg-amber-50 text-amber-700 ring-amber-600/20', 'dot' => 'bg-amber-500'],
        'down' => ['label' => 'Down', 'pill' => 'bg-red-50 text-red-700 ring-red-600/20', 'dot' => 'bg-red-500'],
    ];
@endphp
...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route($indexRouteName, ['status' => '']) }}" class="rounded-lg px-3.5 py-2 text-sm font-semibold transition {{ $filters['status'] === '' ? 'bg-indigo-600 text-white shadow-sm' : 'border border-slate-300 bg-white text-slate-600 hover:bg-slate-50' }}">Semua ({{ $stats['total'] }})</a>
            @foreach (['active', 'maintenance', 'down'] as $st)
                <a href="{{ route($indexRouteName, ['status' => $st]) }}"
                    class="rounded-lg px-3.5 py-2 text-sm font-semibold capitalize transition {{ $filters['status'] === $st ? ($st === 'down' ? 'bg-red-600 text-white shadow-sm' : ($st === 'maintenance' ? 'bg-amber-500 text-white shadow-sm' : 'bg-emerald-600 text-white shadow-sm')) : 'border border-slate-300 bg-white text-slate-600 hover:bg-slate-50' }}">
                    {{ ucfirst($st) }} ({{ $stats[$st] }})
                </a>
            @endforeach
        </div>
        <button type="button" onclick="openModal('modal-add-node')" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-indigo-600/30 transition hover:bg-indigo-500">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-3">
                        <span class="flex h-10 w-10 items-center justify-center rounded-lg {{ $n->status === 'active' ? 'bg-emerald-100 text-emerald-600' : ($n->status === 'maintenance' ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-600') }}">
                        ...
                        <div>
                            <p class="font-mono text-sm font-bold text-slate-900">{{ $n->name }}</p>
                            <p class="text-xs text-slate-500">{{ $n->location_area }}</p>
                        </div>
                    </div>
                    <span class="inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold ring-1 ring-inset {{ $nodeMeta[$n->status]['pill'] }}">
                ...
                <div class="mt-4 flex gap-2">
                    <button type="button" data-node-edit data-url="{{ route('network-nodes.update', $n) }}"
                        ...
                        class="w-full rounded-lg border border-indigo-300 bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700 transition hover:bg-indigo-100">
                    ...
                    @if ($isAdmin)
                        <form method="POST" action="{{ route('network-nodes.destroy', $n) }}" onsubmit="return confirm('Hapus node {{ $n->name }}?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" title="Hapus" class="rounded-lg border border-slate-200 p-2 text-slate-400 transition hover:bg-red-50 hover:text-red-500">
    ...
    <div class="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 px-5 py-4">
            <h3 class="text-base font-bold text-slate-900">Rekapitulasi Tabel Node</h3>
            <p class="text-xs text-slate-500">Sumber data: tabel network_nodes (MySQL) · total {{ $nodes->count() }} node tampil</p>
        </div>
        <div class="w-full overflow-x-auto">
            <table class="min-w-[800px] divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Nama Node</th>
                        <th class="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Lokasi Wilayah</th>
                        <th class="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">IP Address</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($nodes as $n)
                        <tr class="transition hover:bg-slate-50/70">
                            <td class="whitespace-nowrap px-5 py-3.5 font-mono text-sm font-bold text-slate-800">{{ $n->name }}</td>
                            <td class="whitespace-nowrap px-5 py-3.5 text-sm text-slate-600">{{ $n->location_area }}</td>
                            <td class="whitespace-nowrap px-5 py-3.5">@if ($n->ip_address)<span class="font-mono text-xs text-slate-600">{{ $n->ip_address }}</span>@else<span class="text-xs italic text-slate-400">belum diset</span>@endif</td>
                            <td class="whitespace-nowrap px-5 py-3.5 text-sm font-semibold text-slate-700">{{ $n->active_customers }}</td>
                            <td class="whitespace-nowrap px-5 py-3.5">
                                <span class="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold ring-1 ring-inset {{ $nodeMeta[$n->status]['pill'] }}">
                                    <span class="h-1.5 w-1.5 rounded-full {{ $nodeMeta[$n->status]['dot'] }}"></span>{{ $nodeMeta[$n->status]['label'] }}
                                </span>
                            </td>
                            <td class="whitespace-nowrap px-5 py-3.5 text-right"><button type="button" onclick="document.querySelectorAll('[data-node-edit]').forEach(function(b){ if(b.dataset.name === '{{ $n->name }}') b.click(); })" class="font-semibold text-indigo-600 hover:text-indigo-500">Edit</button></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
@push('modals')
    <div id="modal-add-node" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between border-b border-slate-200 px-6 py-4">
                <div>
                    <h3 class="text-base font-bold text-slate-900">Tambah Network Node</h3>
            ...
            <form method="POST" action="{{ route('network-nodes.store') }}" class="space-y-4 px-6 py-5">
                ...
                <div class="flex justify-end gap-2 border-t border-slate-200 pt-4">
                    <button type="button" onclick="closeModal('modal-add-node')" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>
                    <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Simpan Node</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-edit-node" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between border-b border-slate-200 px-6 py-4">
                <div>
                    <h3 class="text-base font-bold text-slate-900">Edit Node</h3>
            ...
            <form id="edit-node-form" method="POST" action="#" class="space-y-4 px-6 py-5">
                ...
                @include('nodes._fields')
                <div class="flex justify-end gap-2 border-t border-slate-200 pt-4">
                    <button type="button" onclick="closeModal('modal-edit-node')" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>
                    <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
